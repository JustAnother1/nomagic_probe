...
void console_uart_initialize(void) {
    ...
    RESETS->RESET = RESETS->RESET & ~(1 << 18);  // sysCFG
    while (0 == ((1 << 18) & RESETS->RESET_DONE)) {
        ;
    }

    RESETS->RESET = RESETS->RESET & ~(1 << 22);
...
bool console_uart_get_received_bytes(uint8_t *buf, uint32_t length) {
    uint32_t i;
    if (length < console_uart_get_num_received_bytes()) {
        return false;
    }
    for (i = 0; i < length; i++) {
        buf[i] = console_uart_get_next_received_byte();
    }
    return true;
}
